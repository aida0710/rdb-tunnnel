use crate::packet_reassembly::ip_header::{parse_ip_header, IpHeader};
use crate::packet_reassembly::ip_reassembly::IpReassembler;
use crate::packet_reassembly::tcp_header::{parse_tcp_header, parse_tcp_options};
use crate::packet_reassembly::tcp_stream::{TcpStream, TcpStreamKey, TCP_SYN};
...
fn process_tcp_header_and_payload(
    ip_header: &IpHeader,
    tcp_header: &crate::packet_reassembly::tcp_header::TcpHeader,
    payload: &[u8],
    streams: &mut HashMap<TcpStreamKey, TcpStream>,
    arrival_time: SystemTime,
) -> Result<(), Box<dyn std::error::Error>> {
...
fn process_tcp_data(
    ...
    if let Some(stream) = streams.get_mut(&stream_key) {
        ...
        if stream.state == crate::packet_reassembly::tcp_stream::TcpState::Closed {
